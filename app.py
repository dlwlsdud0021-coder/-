...
def render_home():
    ...
    desc_full = f"{desc} · S&P500 {'+' if sp_pct>=0 else ''}{sp_pct:.2f}% / 나스닥 {'+' if nd_pct>=0 else ''}{nd_pct:.2f}%"
...
# 탭바를 body로 이동 + MutationObserver로 Streamlit 리렌더 후에도 유지
...
(function() {
  ...
  function ensureStyle() {
    if (p.getElementById('__tabnav_style__')) return;
  ...
  function moveTabBar() {
    ...
    if (!tabList) return;
    if (tabList.parentElement === p.body) return;
    _moving = true;
  ...
  function init() {
    ensureStyle();
    moveTabBar();
    // Streamlit이 리렌더할 때마다 탭바 되돌아오는 것 감시
    var obs = new MutationObserver(function() {
      if (!_moving) moveTabBar();
    });
    obs.observe(p.body, { childList: true, subtree: true });
  }

  if (p.readyState === 'loading') { p.addEventListener('DOMContentLoaded', init); }
  else { init(); }
  setTimeout(init, 500);
})();
</script>
