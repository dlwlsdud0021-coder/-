...
def generate_ai_summary(title, summary, sentiment, category):
    ...
    prompt = (
        ...
        "⚠️ 중요: 기사 제목과 본문에 나온 사실만 바탕으로 분석하세요. "
        "기사에 없는 내용을 추론하거나 반대 방향으로 해석하지 마세요. "
        "예를 들어 '금리 인상' 기사라면 금리 인하로 해석하지 마세요.\n\n"
        "기사 제목: " + title + "\n"
        + body_part +
        "분야: " + category_label + "\n\n"
        "[핵심요약]\n"
        "이 뉴스가 무슨 내용인지 4~5줄로 완전한 문장으로 요약하세요.\n"
        "- 기사 제목과 본문의 핵심 사실을 그대로 반영하세요\n"
        "- 누가, 무엇을, 왜, 어떤 결과가 예상되는지 포함\n"
        "- 숫자(금액, 비율, 날짜)가 있으면 반드시 언급\n"
        "- 반드시 완전한 문장으로 끝내세요 (말줄임표 '...' 금지)\n"
        "- 전문용어는 괄호로 쉽게 설명 (예: QE(양적완화))\n\n"
        "[영향종목]\n"
        "기사 내용을 바탕으로 실제로 영향받는 주식 종목을 작성하세요.\n"
        "▲ 수혜 종목: 종목명 — 이유 (이 기사 때문에 왜 오를 수 있는지)\n"
        "▼ 피해 종목: 종목명 — 이유 (이 기사 때문에 왜 내릴 수 있는지)\n"
        "종목을 특정하기 어려우면 ETF(예: KODEX 200, TIGER 반도체)로 대체 가능\n\n"
        "[투자전략]\n"
        "이 기사의 실제 내용에 근거한 전략 3줄로 작성하세요 (기사 내용과 반드시 일치해야 함).\n"
        "1. 지금 상황 판단: 이 뉴스가 시장에 호재인지 악재인지 판단과 이유\n"
        "2. 행동 지침: 보유자/미보유자 각각 어떻게 할지 (손절가 -X%, 목표가 +X% 형식)\n"
        "3. 주의사항: 이 뉴스 해석 시 반드시 확인해야 할 변수나 리스크\n"
    )
    ...
    impact_detail = {
        ("반도체", "positive"): (
            "삼성전자(005930)와 SK하이닉스(000660) 등 반도체 대형주에 직접적인 매수세가 유입될 가능성이 높습니다. "
            "AI 반도체 수요가 늘어나는 구조이므로 HBM 관련 뉴스는 특히 SK하이닉스에 강하게 반응합니다. "
            "대형주 상승 시 한미반도체·이오테크닉스 등 반도체 장비·소재 중소형주도 동반 상승하는 '낙수 효과'가 나타납니다. "
            "단, 뉴스 발표 직후 이미 주가가 2~3% 이상 올랐다면 추격 매수보다 소화 기간을 기다리는 게 현명합니다."
        ),
        ("반도체", "negative"): (
            "삼성전자·SK하이닉스 주가에 단기 하락 압력이 가해질 수 있습니다. "
            "반도체 업황 악화 뉴스는 단순히 한 기업의 문제가 아니라 섹터 전체의 매도를 유발합니다. "
            "외국인 투자자가 반도체주를 집중 매도하면 코스피 전체도 끌어내릴 수 있으므로 다른 종목 보유자도 주의가 필요합니다. "
            "SOX 지수(미국 반도체 지수)의 움직임을 함께 확인하면 국내 반도체주 방향성을 예측하는 데 도움이 됩니다."
        ),
        ("바이오", "positive"): (
            "임상 성공이나 허가 승인 뉴스는 해당 기업 주가를 하루에 20~50% 이상 끌어올리는 경우도 있습니다. "
            "비슷한 치료 영역의 다른 바이오 기업들도 동반 상승하는 '섹터 랠리'가 나타납니다. "
            "단, 이미 기대감으로 주가가 많이 오른 상태라면 '소문에 사고 뉴스에 팔라'는 격언처럼 차익 실현 매물이 나올 수 있습니다. "
            "허가 여부가 확정되지 않은 '기대감 단계'의 바이오주는 결과 발표 후 급락할 수 있으므로 비중 관리가 필수입니다."
        ),
        ("바이오", "negative"): (
            "임상 실패 뉴스는 해당 기업 주가를 하루에 30~60% 급락시킬 수 있는 최악의 악재입니다. "
            "바이오 섹터 전반 투자심리가 냉각되어 관련 없는 바이오주도 동반 하락하는 경우가 많습니다. "
            "단, 급락 이후 단기 기술적 반등이 나타나는 경우도 있으므로, 섣불리 공매도보다는 상황을 지켜보는 것이 안전합니다. "
            "해당 기업이 다른 파이프라인(신약 후보)을 보유하고 있다면 그 가치를 별도로 평가해봐야 합니다."
        ),
        ("2차전지", "positive"): (
            "LG에너지솔루션·삼성SDI·SK이노베이션 등 배터리 대형주와 에코프로·포스코퓨처엠 등 소재주 전반에 매수세가 유입될 수 있습니다. "
            "전기차 판매량이 늘면 배터리 셀 → 양극재 → 음극재 → 리튬 전 밸류체인이 함께 오르는 구조입니다. "
            "단, 중국 CATL 등 경쟁사와의 가격 경쟁 여부, 배터리 수주 계약 조건도 함께 확인해야 실질적인 수혜 규모를 알 수 있습니다."
        ),
        ("2차전지", "negative"): (
            "배터리 관련주 전반에 매도 압력이 생깁니다. 수요 둔화 뉴스는 배터리 셀 기업뿐 아니라 소재·부품 업체까지 연쇄적으로 타격을 줍니다. "
            "특히 에코프로·포스코퓨처엠 등 소재주는 변동성이 커서 하락폭이 대형주보다 클 수 있습니다. "
            "LG에너지솔루션·삼성SDI 같은 대형주는 ESS(에너지저장장치) 사업 등 전기차 외 수요로 버텨줄 가능성이 있어 소재주보다 방어적입니다."
        ),
        ("금융", "positive"): (
            "금리 인하 기대감은 주식 시장 전반을 끌어올립니다. PER(주가수익비율)이 높은 성장주들이 특히 큰 혜택을 받습니다. "
            "KB금융·신한금융 등 은행주는 단기적으로 순이자마진(NIM) 축소 우려로 오히려 조정받을 수 있으니 주의가 필요합니다. "
            "원화 강세(환율 하락)가 동반되면 외국인이 한국 주식을 더 많이 사서 코스피 상승을 이끌 가능성이 높습니다."
        ),
        ("금융", "negative"): (
            "금리 인상 또는 긴축 우려는 주식 시장 전반에 하락 압력을 줍니다. "
            "돈의 가격(금리)이 오르면 미래 이익을 높게 평가받던 성장주(바이오, 플랫폼, 2차전지)가 가장 많이 떨어집니다. "
            "은행·보험 등 금융주는 상대적으로 방어적이지만, 경기 침체 우려가 함께 부각되면 이들도 하락합니다. "
            "환율 상승(원화 약세)이 동반되면 수입 비중이 높은 기업(항공사, 정유사 등)도 타격을 받습니다."
        ),
        ("글로벌", "positive"): (
            "미국 증시 호조는 다음 날 한국 증시 시초가를 끌어올리는 직접적 효과가 있습니다. "
            "나스닥이 오르면 코스닥 기술주가, S&P500이 오르면 코스피 대형주가 동반 상승하는 패턴이 자주 나타납니다. "
            "외국인이 한국 주식을 사들이면 삼성전자·SK하이닉스·현대차 등 외국인 비중이 높은 종목들이 먼저 오릅니다."
        ),
        ("글로벌", "negative"): (
            "미국 증시 하락은 한국 증시에 직접적인 전염 효과를 냅니다. 나스닥이 2% 이상 하락하면 코스닥도 비슷한 폭으로 빠지는 경우가 많습니다. "
            "외국인이 한국 주식을 팔면 원화 가치도 하락(환율 상승)하여 이중 손실이 발생할 수 있습니다. "
    ...
    # 섹션 3: 대응 방법 (단기/중기 구분)
    _TERM_HEADER_S = "<span style='color:#5B5BD6;font-weight:600;'>▶ 단기(1~2주)</span> "
    _TERM_HEADER_M = "<br><span style='color:#FF9F0A;font-weight:600;'>▶ 중기(1~3개월)</span> "

    if sentiment == "positive":
        if co_str:
            action = (
                _TERM_HEADER_S +
                f"<b>보유자:</b> {co_str} 보유 중이라면 보유 유지하세요. 거래량이 평소의 2배 이상이면 1차 목표가(+8%) 도달 시 보유량의 절반 익절을 고려하세요. "
                f"<b>미보유자:</b> 갭 상승이 +3% 이내면 투자금의 20~30%를 1차 매수. +5% 이상 갭업 출발 시 추격 매수 금지, 2~3일 소화 후 눌림목(-3~5%) 기다리세요. "
                f"<b>손절선:</b> 매수가 기준 <b>-5%</b>(예: 10만원 매수 → 9만 5천원 이탈 시 즉시 손절). 절대 지키는 것이 핵심입니다."
                +
                _TERM_HEADER_M +
                f"추세가 1~2주 이상 유지되면 2차 매수(추가 20~30%)로 비중을 50~60%까지 확대하세요. "
                f"중기 목표가는 <b>+15~20%</b>. 단, 주가가 20일 이동평균선을 3일 연속 하회하면 비중을 절반으로 줄이는 것이 안전합니다. "
                f"외국인 순매수 지속 여부를 매주 확인하며 추세 유지 판단을 하세요."
            )
        else:
            action = (
                _TERM_HEADER_S +
                f"섹터 대형주(1등주)는 이미 반영됐을 수 있으니 2~3순위 종목을 탐색하세요. "
                f"KODEX 반도체·TIGER 2차전지 등 섹터 ETF를 투자금의 10~20% 소량 매수하면 리스크 분산 효과가 있습니다. "
                f"<b>손절선:</b> 매수가 대비 <b>-5%</b> 이탈 시 무조건 손절. 분할 매수(1차 30% → 2차 30% → 3차 40%) 원칙을 지키세요."
                +
                _TERM_HEADER_M +
                f"섹터 업황이 3~4주 이상 긍정 흐름을 유지할 때 비중을 포트폴리오의 20~30%까지 늘리는 것이 적절합니다. "
                f"중기 목표가는 <b>+15%</b>이며, 섹터 내 외국인 순매수가 3주 연속 줄어들면 비중을 재검토하세요."
            )
    elif sentiment == "negative":
        if co_str:
            action = (
                _TERM_HEADER_S +
                f"<b>보유자:</b> 매수 평균 단가보다 <b>-5%</b> 아래(예: 10만원 매수 → 9만 5천원)를 손절선으로 설정하세요. "
                f"이미 -5% 이상 손실이라면 추가 매수(물타기)보다 손절 후 재진입이 안전합니다. "
                f"<b>미보유자:</b> 악재 발표 후 2~3일은 매수 금지. 하락 속도가 줄어들고 거래량이 감소하는 '바닥 신호' 확인 후 소량(10~15%) 진입을 검토하세요."
                +
                _TERM_HEADER_M +
                f"악재의 구조적 지속 여부를 판단하세요. 일회성 악재(단기 이슈)라면 4~6주 후 저점 매수 기회가 올 수 있습니다. "
                f"구조적 악재(업황 악화)라면 해당 종목 비중을 포트폴리오의 5% 이하로 제한하세요. "
                f"중기 재진입 기준: 20일선 회복 + 외국인 순매수 전환이 동시에 나타날 때."
            )
        else:
            action = (
                _TERM_HEADER_S +
                f"<b>보유자:</b> 해당 섹터 보유 종목 손절선(<b>-5~7%</b>)을 오늘 즉시 재확인하세요. 현금 비중을 전체의 <b>30% 이상</b>으로 높이는 것을 권장합니다. "
                f"<b>미보유자:</b> 지금은 기다리세요. 외국인·기관 순매수 전환이 확인된 후 재진입을 검토하세요."
                +
                _TERM_HEADER_M +
                f"하락 추세가 4주 이상 지속되면 해당 섹터 비중을 포트폴리오의 10% 이하로 줄이세요. "
                f"업황 회복 신호(수주 증가, 재고 감소, 실적 개선 전망)가 2가지 이상 나타날 때 비중을 단계적으로 회복시키세요. "
                f"회복 진입 시 손절선은 <b>-7%</b>로 초기보다 빡빡하게 설정하는 것이 안전합니다."
            )
    elif sentiment == "mixed":
        action = (
            _TERM_HEADER_S +
            "장 초반(9시~9시 30분) 주가와 거래량을 먼저 확인하세요. "
            "상승+거래량 증가 → 긍정 해석, 하락+거래량 증가 → 부정 해석으로 판단하세요. "
            "신규 진입 시 투자 규모를 평소의 <b>50% 이하</b>로 제한하고, 손절선은 <b>-5%</b>로 단기 설정하세요."
            +
            _TERM_HEADER_M +
            "방향성이 명확해지는 2~3주 후 본격적인 포지션을 구성하세요. "
            "긍정 방향으로 확정되면 비중을 30%까지 확대, 부정 방향이면 손절 후 섹터 교체를 검토하세요. "
            "판단 기준: 20일선 위/아래 정착 여부 + 외국인 수급 방향."
        )
    else:
        action = (
            _TERM_HEADER_S +
            "뚜렷한 방향성이 없는 구간입니다. 급하게 사거나 팔 필요가 없습니다. "
            "보유 중인 종목의 손절선(<b>-5%</b>) 설정 여부를 점검하는 데 집중하세요."
            +
            _TERM_HEADER_M +
            "다음 실적 발표 또는 주요 이벤트(금리 결정, 수주 공시 등) 일정을 캘린더에 기록해두고, "
            "이벤트 결과를 확인한 후 방향성에 맞게 포지션을 구성하세요. "
            "현재는 현금 비중(<b>30~40%</b>) 유지가 최선의 전략입니다."
        )

    return (
        "<b>📋 무슨 뉴스인가요?</b><br>" + what + "<br><br>"
        "<b>📊 주가에 어떤 영향이 있나요?</b><br>" + impact + "<br><br>"
        "<b>💬 어떻게 대응하면 좋을까요?</b><br>" + action
    )
